...
class OnboardingPreferences {
  ...
  static Future<void> setOnboardingComplete(
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (success) {
          _logger.info(
            'Saved isOnboardingComplete to SharedPreferences successfully.',
          );
          return;
        } else {
          throw Exception(
  ...
  static Future<bool> isOnboardingComplete() async {
    try {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      final bool result =
          prefs.getBool(_kOnboardingComplete) ?? kDefaultOnboardingComplete;
      _logger.info('Retrieved isOnboardingComplete: $result');
      return result;
    } catch (e) {
      _logger.warning(
        'Failed to get isOnboardingComplete from SharedPreferences: $e',
      );
      return false;
    }
  }
