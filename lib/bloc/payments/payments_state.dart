...
import 'models/payment_filters.dart';
...
class PaymentsState {
  ...
  PaymentsState.initial()
    : this(payments: <PaymentData>[], paymentFilters: PaymentFilters.initial());
  ...
  PaymentsState copyWith({
    ...
    final List<PaymentData>? prefilteredPayments =
        payments?.where((PaymentData paymentData) {
          final String? paymentAssetTicker = paymentData.details.map(
            liquid:
                (PaymentDetails_Liquid details) =>
                    details.assetInfo?.ticker ?? '',
            orElse: () => null,
          );
  ...
  List<PaymentData> get filteredPayments {
    ...
    final Set<String>? typeFilterSet =
        paymentFilters.hasTypeFilters
            ? paymentFilters.filters!
                .map((PaymentType filter) => filter.name)
                .toSet()
            : null;
    ...
    return payments.where((PaymentData paymentData) {
      ...
      final bool passDateFilter =
          ...
          (paymentFilters.fromTimestamp! < milliseconds &&
              milliseconds < paymentFilters.toTimestamp!);

      final bool passTypeFilter =
          typeFilterSet == null ||
          typeFilterSet.contains(paymentData.paymentType.name);

      final String? paymentAssetTicker = paymentData.details.map(
        liquid:
            (PaymentDetails_Liquid details) => details.assetInfo?.ticker ?? '',
        orElse: () => null,
      );
  ...
  Map<String, dynamic> toJson() {
    return <String, dynamic>{
      'payments':
          payments.map((PaymentData payment) => payment.toJson()).toList(),
  ...
  factory PaymentsState.fromJson(Map<String, dynamic> json) {
    return PaymentsState(
      payments:
          (json['payments'] as List<dynamic>? ?? <PaymentData>[])
