...
import 'package:bitwit_shit/models/payment_details_extension.dart';
import 'package:bitwit_shit/models/payment_title_extension.dart';
import 'package:bitwit_shit/utils/enum_utils.dart';
...
class PaymentData {
  ...
  factory PaymentData.fromPayment(Payment payment) {
    final _PaymentDataFactory factory = _PaymentDataFactory(payment);
  ...
  int get refundTxAmountSat => details.map(
    bitcoin:
        (PaymentDetails_Bitcoin details) =>
            details.refundTxAmountSat?.toInt() ?? 0,
    lightning:
  ...
  bool get isRefunded =>
      (refundTxAmountSat > 0 || refundTxId != null) &&
      status == PaymentState.failed;

  int get actualFeeSat =>
      (isRefunded && refundTxAmountSat > 0)
          ? amountSat - refundTxAmountSat
          : feeSat;
...
class _PaymentDataFactory {
  ...
  _PaymentDataFactory(this._payment);
  ...
  String _title() {
    ...
    if (lnurlInfo != null) {
      final Map<String, dynamic> metadataMap = getLnurlPayMetadata(
        _payment.details,
      );
      final String? lnUrlTitle =
          lnurlInfo.lnAddress?.isNotEmpty == true
              ? lnurlInfo.lnAddress
...
Map<String, dynamic> getLnurlPayMetadata(PaymentDetails details) {
  final String? lnurlPayMetadata = details.map(
    lightning:
        (PaymentDetails_Lightning details) =>
            details.lnurlInfo?.lnurlPayMetadata,
    orElse: () => null,
  );
  ...
  try {
    ...
    return Map<String, dynamic>.fromEntries(
      decoded
          ...
          .map(
            (List<dynamic> item) =>
                MapEntry<String, dynamic>(item[0] as String, item[1]),
          ),
    );
  } catch (_) {
    return <String, dynamic>{};
