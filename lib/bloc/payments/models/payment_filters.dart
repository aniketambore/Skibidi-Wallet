...
class PaymentFilters implements Exception {
  ...
  bool get hasTypeFilters =>
      filters != null && !listEquals(filters, (PaymentType.values));
        ...
        for (final dynamic filter in filtersValue) {
          if (filter is String) {
            ...
            final String enumValue =
                filter.contains('.') ? filter.split('.').last : filter;
  ...
  Map<String, dynamic> toJson() {
    final List<String>? filtersJson =
        filters?.map((PaymentType type) => type.name).toList();
