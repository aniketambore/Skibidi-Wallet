...
class BreezDateUtils {
  ...
  static final DateFormat _yearMonthDayFormat = DateFormat.yMd(
    Platform.localeName,
  );
  ...
  static final DateFormat _hourMinuteDayFormat = DateFormat.jm(
    Platform.localeName,
  );
  ...
  static String formatYearMonthDayHourMinute(
    DateTime d,
  ) => _yearMonthDayHourMinuteFormat
      .format(d)
  ...
  static String formatTimelineRelative(DateTime d) {
    if (DateTime.now().subtract(const Duration(days: 4)).isBefore(d)) {
      return timeago.format(d, locale: 'en');
    } else {
  ...
  static DateTime? _blockDiffToDate({
    ...
    if (blockHeight == null ||
        expiryBlock == null ||
        blockHeight > expiryBlock) {
      _logger.fine(
        'Invalid block parameters: height=$blockHeight, expiry=$expiryBlock',
      );
      return null;
    }
  ...
  /// Calculates an estimated date from Liquid block heights
  ///
  ...
  static DateTime? liquidBlockDiffToDate({
    ...
    return _blockDiffToDate(
      blockHeight: blockHeight,
      expiryBlock: expiryBlock,
      secondsPerBlock: BlockTimes.liquidBlockTimeSeconds,
    );
  }
